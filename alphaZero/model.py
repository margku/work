...
class Gen_Model():
	...
	def write(self, game, version):
		...
		Provider.addNet(name, env.name, now)
		self.model.save(run_folder + 'models/'+ env.name + "{0:0>4}".format(version) + '.h5')
...
# 		return load_model( run_archive_folder + game + '/run' + str(run_number).zfill(4) + "/models/version" + "{0:0>4}".format(version) + '.h5', custom_objects={'softmax_cross_entropy_with_logits': softmax_cross_entropy_with_logits})
		return load_model(run_folder + 'models/' + env.name + "{0:0>4}".format(version) + '.h5', custom_objects = {'softmax_cross_entropy_with_logits': softmax_cross_entropy_with_logits})
