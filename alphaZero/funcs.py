...
def playMatchesBetweenVersions(env, run_version, player1version, player2version, EPISODES, logger, turns_until_tau0, goes_first = 0):
    ...
    if player1version == -1:
        ...
        player1 = Agent( 'user1', env.state_size, env.action_size, config.MCTS_SIMS, config.CPUCT, player1_NN)

    if player2version == -1:
        player2 = User('user2', env.state_size, env.action_size)
    else:
        ...
        player2 = Agent('user2', env.state_size, env.action_size, config.MCTS_SIMS, config.CPUCT, player2_NN)
