...
import ru.sfedu.postHibernate.models.tablePerClass.Letter;
import ru.sfedu.postHibernate.models.tablePerClass.Money;
import ru.sfedu.postHibernate.models.tablePerClass.ValuablePack;
import ru.sfedu.postHibernate.provider.iDataProvider.IDataProviderHibernate;
import ru.sfedu.postHibernate.utils.HibernateUtilTPC;
...
import static ru.sfedu.postHibernate.Constants.IDP_FAIL;
import static ru.sfedu.postHibernate.Msg.COMPLETE;
import static ru.sfedu.postHibernate.Msg.NO;

public class DataProviderTablePerClass implements IDataProviderHibernate {
    private static Logger log = LogManager.getLogger(DataProviderTablePerClass.class);
    ...
    public Result addMoney(long sender, long recipient, String addressFrom, long postOfficeFrom, String addressTo, long postOfficeTo, int price, String date, int sum, String currency) {
        try {
            ...
            Session session = HibernateUtilTPC.getSessionFactory().openSession();
    ...
    public Money getMoney(long id) {
        return HibernateUtilTPC.getSessionFactory().openSession().get(Money.class, id);

    }

    @Override
    public ValuablePack getValPack(long id) {
        return HibernateUtilTPC.getSessionFactory().openSession().get(ValuablePack.class, id);
    }

    @Override
    public Letter getLetter(long id) {
        return HibernateUtilTPC.getSessionFactory().openSession().get(Letter.class, id);
    }
    ...
    public List<?> getListOfCreatedTables() {
        ...
        try {
            ...
            Query query = session.getNamedNativeQuery(HSQLDB_CATALOG_TABLEPERCLASS);
